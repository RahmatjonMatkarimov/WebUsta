      ...
      <div class="flex items-center justify-between h-14 sm:h-16">
        ...
        <NuxtLink to="/" class="flex items-center gap-2 group flex-shrink-0" aria-label="WebUsta bosh sahifa">
          <div class="h-28 sm:h-36 overflow-hidden flex items-center">
            <img :src="logoSrc" alt="WebUsta Logo" class="h-full w-auto logo-img" />
          </div>
        </NuxtLink>
        ...
        <ul class="hidden lg:flex items-center gap-0.5 xl:gap-1" role="list">
          <li v-for="item in navItems" :key="item.href">
            <a :href="item.href"
              class="px-3 py-2 rounded-lg text-sm font-medium text-[var(--color-text-muted)] hover:text-[var(--color-text)] hover:bg-[var(--color-bg-muted)]">
              {{ item.label }}
            </a>
          </li>
        </ul>
        ...
        <div class="flex items-center gap-1.5 sm:gap-2">
          ...
          <button @click="toggleDark"
            class="p-2 rounded-lg text-[var(--color-text-muted)] hover:text-[var(--color-text)] hover:bg-[var(--color-bg-muted)] touch-target flex items-center justify-center"
          ...
          <button @click="mobileOpen = !mobileOpen"
            class="lg:hidden p-2 rounded-lg text-[var(--color-text-muted)] hover:bg-[var(--color-bg-muted)] touch-target flex items-center justify-center"
        ...
        <ul class="flex flex-col gap-1" role="list">
          <li v-for="item in navItems" :key="item.href">
            <a :href="item.href" @click="mobileOpen = false"
              class="block px-3 py-2.5 rounded-lg text-sm font-medium text-[var(--color-text-muted)] hover:text-[var(--color-text)] hover:bg-[var(--color-bg-muted)]">
              {{ item.label }}
            </a>
          </li>
          <li class="mt-2">
